! function(a) {
    a.fn.niceSelect = function(d) {
        function c(h) {
            h.after(a("<div></div>").addClass("nice-select").addClass(h.attr("class") || "").addClass(h.attr("disabled") ? "disabled" : "").attr("tabindex", h.attr("disabled") ? null : "0").html('<span class="current"></span><ul class="list"></ul>'));
            var g = h.next(),
                f = h.find("option"),
            ...
            g.find(".current").html(e.data("display") || e.text()), f.each(function(l) {
                var k = a(this),
                    j = k.data("display");
                g.find("ul").append(a("<li></li>").attr("data-value", k.val()).attr("data-display", j || null).addClass("option" + (k.is(":selected") ? " selected" : "") + (k.is(":disabled") ? " disabled" : "")).html(k.text()))
            })
        }
        if ("string" == typeof d) {
